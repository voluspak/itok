...
import { VIDEO_POOL_SIZE, MIN_CACHE_SIZE, DEFAULT_TTL, RAPIDAPI_HOST, RAPIDAPI_BASE_URL, REGIONS } from '../../const'
...
async function fetchRapid(path) {
  const url = `${RAPIDAPI_BASE_URL}${path}`
  const options = {
    method: 'GET',
    headers: {
...
async function fetchFeedAndRefreshCache(regions) {
  if (isFetching) {
    ...
    // Esperar hasta que termine el fetch actual (max 10 segundos)
    let attempts = 0
    // eslint-disable-next-line no-unmodified-loop-condition
    while (isFetching && attempts < 100) {
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const url = `${RAPIDAPI_BASE_URL}/feed/list?region=${region}&count=15`
      try {
        const response = await fetch(url, options)
        if (!response.ok) {
...
export default async function handler(req, res) {
  try {
    ...
    if (action === 'list') {
      // Si tenemos al menos MIN_CACHE_SIZE videos, retornarlos sin llamar a la API
      if (videoCache.length >= MIN_CACHE_SIZE) {
        console.log(`Usando caché (${videoCache.length} videos disponibles, age: ${Math.round((now - lastFetch) / 1000)}s)`)

        const videosToReturn = Math.min(8, videoCache.length)
      ...
      // Solo llamar a la API si el cache tiene menos de MIN_CACHE_SIZE videos
      if (!isFetching) {
        console.log(`Caché insuficiente (${videoCache.length} videos), llamando a la API...`)
        await fetchFeedAndRefreshCache(REGIONS)
        console.log(`Caché actualizada: ${videoCache.length} videos únicos`)
      }

      // Si después del fetch no hay videos, retornar error
      if (videoCache.length === 0) {
        return res.status(503).json({
          success: false,
          message: 'No hay videos disponibles en este momento. Intente nuevamente.',
          cache_size: 0
        })
      }
